    ...
    def step(self):
        ...
        if len(see) == 0: # No food, explore or go to memory
            ...
            if len(uMem) == 0:
                ...
                x, y, k = self.memory[k]
                x = max(0, min(self.world.size - 1, x + random.randrange(-2, 3)))
                y = max(0, min(self.world.sizeW - 1, y + random.randrange(-2, 3)))
            ...
            if eG > 4:
                if len(self.memory) == 0:
                    self.memory.append([x[0], x[1], self.revisit])
                elif len(self.memory) < 3:
                    if min([self._dist_from(i, j) for i, j, k in self.memory]) < 5:
                        pass
                    else:
                        self.memory.append([x[0], x[1], self.revisit])
                else:
