        ...
        if len(seeAgents) != 0:
            ...
            if len(agComm.memory) > 0:
                for i in np.arange(agComm.communication):
                    if random.random() < self.commProb:
                        self.memoryProcess(random.choice(agComm.memory))
